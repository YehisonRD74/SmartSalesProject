...
namespace SmartSales.Data.Configuration
{
    public class DbConnectionStringProvider : IDbConnectionStringProvider
    {
        ...
        public async Task EnsureDatabaseExistsAsync(string? connectionString = null)
        {
            var effectiveConnection = string.IsNullOrWhiteSpace(connectionString)
                ? ConnectionString
                : connectionString;

            var targetBuilder = new SqlConnectionStringBuilder(effectiveConnection);
            ...
            var masterBuilder = new SqlConnectionStringBuilder(effectiveConnection)
            {
...
END", masterConnection);
            ...
            await using var targetConnection = new SqlConnection(effectiveConnection);
            await targetConnection.OpenAsync();

            await EnsureSchemaAsync(targetConnection);
        }
